...
interface CreatableSelectProps {
  ...
  placeholder?: string;
  entityType?: 'customer' | 'supplier' | 'account' | 'product' | string;
  disabled?: boolean;
  className?: string;
  compact?: boolean;
}
...
export default function CreatableSelect({
  ...
  return (
    <div className={`relative w-full ${className}`} ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-[10000] top-full left-0 right-0 mt-1.5 min-w-[200px] bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden animate-in fade-in duration-150">
          <div className="p-2 border-b border-gray-100 bg-gray-50">
            <input
              ...
              className="w-full px-2.5 py-1.5 text-xs border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-600 bg-white"
            />
          </div>
          ...
          <div className="max-h-60 overflow-y-auto p-1 space-y-0.5">
            ...
            {filteredOptions.map(option => (
              <button
                key={option.id || 'custom-empty-option'}
                type="button"
                onClick={() => {
                  onChange(option.id);
                  setIsOpen(false);
                ...
                className={`w-full px-2.5 py-1.5 text-xs text-left rounded-lg flex items-center justify-between gap-2 transition-colors cursor-pointer ${
                  value === option.id
