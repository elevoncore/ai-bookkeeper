...
export default function SalesHub() {
 ...
 const [paymentData, setPaymentData] = useState({ invoice_id: '', amount: '', date: new Date().toISOString().split('T')[0], method: 'Bank Transfer', payment_account_id: '' });
 ...
 const lineItemsPayload = invoiceLines.map(line => ({
    ...
    total: Number(line.quantity || 1) * Number(line.unit_price || 0)
  }));
...
async function handleLogCustomerAdvance(e: React.FormEvent) {
 ...
 return products.filter(p => p.name?.toLowerCase().includes(lower));
  ...
  async function openEditModal(inv: any) {
    setIsEditing(true);
    setNewInvoice({
      id: inv.id,
 ...
 <div>
        ...
        <div key={index} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center bg-gray-50 p-3 rounded-xl border border-gray-200">
          ...
          <div className="pt-4 sm:pt-0">
            <button
              ...
              onClick={() => setInvoiceLines(invoiceLines.filter((_, idx) => idx !== index))}
    ...
    <div className="pt-2 mt-2 flex justify-between items-center text-sm font-bold border-t border-gray-200">
      <span>Invoice Total:</span>
      <span className="text-blue-700 text-base">{calculatedTotal.toLocaleString()} PKR</span>
    </div>
  </div>
 ...
 <div className="p-4 sm:p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3 shrink-0">
 <button type="button" onClick={() => setIsPaymentModalOpen(false)} disabled={isSubmitting} className="px-4 py-2.5 min-h-[44px] border border-gray-200 text-gray-700 font-semibold rounded-xl hover:bg-gray-100 transition-colors cursor-pointer disabled:opacity-50 text-sm">
 Cancel
 </button>
            ...
            <form id="customerAdvanceForm" onSubmit={handleLogCustomerAdvance} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 font-medium bg-white">
              ...
              <div>
                ...
                <CreatableSelect
                  ...
                  onChange={(id) => setAdvanceData({ ...advanceData, customer_id: id })}
