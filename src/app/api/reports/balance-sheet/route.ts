...
export async function GET(request: Request) {
  try {
    ...
    // 1. Fetch Accounts with Parent Hierarchy
    const { data: accounts, error: accErr } = await supabase
      .from('accounts')
      .select('id, name, type, is_system, is_cash_account, parent_account_id, parent_id, code')
      .eq('user_id', user.id)
    ...
    equityAccounts.push({
      ...
      code: '-',
      name: "Owner's Net Earnings (Current Period)",
      type: 'equity',
      is_net_income: true,
      balance: netIncome
    });
