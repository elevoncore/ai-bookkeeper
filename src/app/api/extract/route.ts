...
export async function POST(request: Request) {
  try {
    ...
    4. Chart of Accounts Grounding & Account Categorization (CRITICAL): You MUST categorize each line item using ONLY the exact account names provided: [${accountNames}].
       - INVENTORY PURCHASES & STOCK (CRITICAL): When a user buys physical inventory, stock, or items intended for resale (e.g. "Bought 10 laptops for inventory", "Purchased stock"), you MUST set account_name to 'Inventory Asset'. Buying inventory is an Asset exchange (Debit Inventory Asset, Credit Cash/AP) and MUST NEVER be categorized as 'Cost of Goods Sold' or 'General Operating Expense' at purchase time! Net profit MUST NOT decrease when inventory is purchased.
