...
export async function POST(request: Request) {
  try {
    ...
    4. Chart of Accounts Grounding & Expense Categorization (CRITICAL): You MUST categorize each line item using ONLY the exact account names provided: [${accountNames}]. When categorizing an expense, you must select the most appropriate account from this exact list: ['Cost of Goods Sold', 'Rent Expense', 'Utilities', 'Software & Hosting', 'General Operating Expense']. For example, map server/domain costs to 'Software & Hosting', electricity/water to 'Utilities', and office space to 'Rent Expense'. You must place the exact account name in the "account_name" field of each line item.
