...
export default function DashboardPage() {
  ...
  async function fetchInitialData() {
    ...
    // 2. Fire default accounts initialization asynchronously in background (non-blocking)
    (async () => {
      try {
    ...
    // 3. Parallelize Accounts, Invoices, and Bills queries
    try {
      const [
        { data: accounts, error: accError },
        { data: invoicesData, error: invError },
        { data: billData, error: billsError }
      ] = await Promise.all([
        supabase.from('accounts').select('*'),
        supabase.from('invoices').select('*, customers(name)').eq('user_id', currentUserId).order('created_at', { ascending: false }),
        supabase.from('bills').select('*, suppliers(name)').eq('user_id', currentUserId).order('created_at', { ascending: false })
      ]);
      ...
      else if (invoicesData) setInvoices(invoicesData.filter((i: any) => i.is_ai_verified));

      if (billsError) console.error("Error fetching bills:", billsError);
      else if (billData) setBills(billData.filter((b: any) => b.is_ai_verified));
      ...
      console.error("Error in parallel fetchInitialData:", err);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function fetchFinancials(uid?: string) {
    ...
    try {
      const [
        ...
        supabase.from('invoices').select('*, customers(name)').eq('user_id', activeUserId).order('created_at', { ascending: false }),
        supabase.from('bills').select('*, suppliers(name)').eq('user_id', activeUserId).order('created_at', { ascending: false })
      ]);
      ...
      console.error("Error in parallel fetchFinancials:", err);
    }
  }
