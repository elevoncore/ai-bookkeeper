...
BEGIN
  -- Validate p_lines is a JSON array
  IF p_lines IS NULL OR jsonb_array_length(p_lines) = 0 THEN
    RAISE EXCEPTION 'Journal entry must contain at least one line item.';
  END IF;

  -- Loop over lines to calculate total debits and credits in cents
  ...
  -- Enforce Double-Entry Balancing Rule (Debits = Credits)
  ...
  -- Create Parent Journal Entry Record
  ...
  -- Insert Child Journal Line Records
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines)
  LOOP
